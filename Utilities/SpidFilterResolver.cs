...
public static class SpidFilterResolver
{
    ...
    ILogger? logger)
  {
    ...
    foreach (var expr in formFilters.Expressions)
    {
      foreach (var part in expr.Parts)
      {
        ...
        if (TryResolveFormFilterByFormKey(part,
              linkCache,
              formIdCache,
              npcFilters,
              factionFilters,
        ...
        if (TryResolveAndAddFilter<IFactionGetter>(part.Value,
              part.IsNegated,
              linkCache,
              factionFilters,
              resolvedEditorIds))
    ...
    foreach (var exclusion in formFilters.GlobalExclusions)
    {
      ...
      if (TryResolveFormFilterByFormKey(exclusionPart,
            linkCache,
            formIdCache,
            npcFilters,
            factionFilters,
    ...
    ILogger? logger)
  {
    if (TryParseAsFormKey(part.Value, out var formKey))
    {
      ...
      return TryResolveByFormKey(formKey,
        part.IsNegated,
        part.Value,
        linkCache,
        npcFilters,
    ...
    if (FormKeyHelper.TryParseFormId(part.Value, out var formId))
    {
      ...
      return TryResolveBareFormId(formId,
        part.IsNegated,
        part.Value,
        linkCache,
        formIdCache,
  ...
  private static bool TryResolveByFormKey(
    ...
    HashSet<string>? resolvedEditorIds,
    ILogger? logger) =>
    TryResolveFormKeyAs<INpcGetter>(formKey,
      isNegated,
      originalValue,
      linkCache,
      npcFilters,
      resolvedEditorIds,
      logger) ||
    TryResolveFormKeyAs<IFactionGetter>(formKey,
      isNegated,
      originalValue,
      linkCache,
      factionFilters,
      resolvedEditorIds,
      logger) ||
    TryResolveFormKeyAs<IRaceGetter>(formKey,
      isNegated,
      originalValue,
      linkCache,
      raceFilters,
      resolvedEditorIds,
      logger) ||
    TryResolveFormKeyAsFormKey<IOutfitGetter>(formKey,
      originalValue,
      linkCache,
      outfitFilterFormKeys,
      resolvedEditorIds,
    ...
    ILogger? logger)
  {
    ...
    if (TryResolveBareFormIdAs<INpcGetter>(formId,
          isNegated,
          originalValue,
          linkCache,
          npcFilters,
          resolvedEditorIds,
          logger) ||
        TryResolveBareFormIdAs<IFactionGetter>(formId,
          isNegated,
          originalValue,
          linkCache,
          factionFilters,
          resolvedEditorIds,
          logger) ||
        TryResolveBareFormIdAs<IRaceGetter>(formId,
          isNegated,
          originalValue,
          linkCache,
          raceFilters,
          resolvedEditorIds,
          logger) ||
        TryResolveBareFormIdAsFormKey<IOutfitGetter>(formId,
          originalValue,
          linkCache,
          outfitFilterFormKeys,
          resolvedEditorIds,
    ...
    logger?.Warning("Could not resolve bare FormID {Value} (0x{FormId:X}) as NPC, Faction, Race, or Outfit",
      originalValue,
      formId);
    return false;
  }
